...
function changeLanguage() {
  ...
  if (lang === "uz") {
    window.location.href = "indedx.html";
  } else if (lang === "ru") {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const username = localStorage.getItem("registeredSurname");

  const logSelect = document.getElementById("log");
  const profileBox = document.getElementById("profileBox");
  const usernameDisplay = document.getElementById("usernameDisplay");
  const nameDisplay = document.getElementById("surnameDisplay");
...
profileBox.onclick = function(event) {
  event.stopPropagation();
  prof.style.display = 'flex';
};

// Hide prof when clicking outside
document.addEventListener('click', function(event) {
  if (!profileBox.contains(event.target) && !prof.contains(event.target)) {
